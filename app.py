...
'''
    ...
    birth_time_accuracy = st.radio('出生時刻の分かり具合', TIME_ACCURACY_OPTIONS, horizontal=True, index=0)
    st.markdown(f'<div class="input-help">選択中：{html.escape(birth_time_accuracy)}</div>', unsafe_allow_html=True)
    ...
    concern_detail = st.text_area('特に重視したいことの補足', placeholder='例: 個人事業主として今後どう進めるべきか、今後1年の流れを重視して見てほしい、など', height=110)
    render_form_gap(2)

    st.markdown('<div class="label-sm">手相の写真</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="input-help">手のひら全体が見える、明るくぶれの少ない写真をお選びください。画像は最大{MAX_IMAGE_FILES}枚までです。</div>', unsafe_allow_html=True)
    uploaded_files = st.file_uploader(f'手相の写真（最大 {MAX_IMAGE_FILES} 枚 / 1枚 {MAX_IMAGE_SIZE_MB}MBまで）', type=['png', 'jpg', 'jpeg'], accept_multiple_files=True)
    ...
    if st.button('🐉 龍神さまのお告げを聞く'):
        ...
        if errors:
            ...
            try:
                ...
                image_meta = [PalmImageMeta(filename=file.name, hand_side=hand_sides[idx]) for idx, file in enumerate(uploaded_files or [])]
                ...
                logger.info('fortune_completed', extra={'env': APP_ENV, 'category_count': len(categories), 'image_count': len(image_parts)})
    ...
    try:
        ...
        st.download_button(label='📜 巫女からの手紙を保存する（PDF）', data=pdf_data, file_name=f'miko_letter_{safe_name}.pdf', mime='application/pdf')
    except Exception as exc:
