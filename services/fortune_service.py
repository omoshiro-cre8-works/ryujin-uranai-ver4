...
{{
  "review_fortune": {{
    ...
    "intro": "1. 前回のお告げの振り返り。前回PDFを短く要約し、今回の相談テーマに関係する部分を優先する",
    "continuing_flow": "2. 照らし合わせの前半。前回のお告げから現在まで続いている流れを短く整理する",
    "current_changes": "2. 照らし合わせの中核。近況メモと、手相画像がある場合は今回アップロードされた画像から読み取れる範囲を重ねて、現在までの変化を整理する",
    "theme_review": "2. 照らし合わせの後半。相談テーマと補足情報を軸に、前回のお告げが今どのような形で現れているかを読み直す",
    "next_3_months": "3. これから3カ月ほど意識したいことや小さな行動。箇条書きに近い短い文で具体化する",
    "one_year_guidance": "4. 1年先に向けて整えていくこと。強く断定せず、方向性を考えやすくする",
    "ryujin_message": "5. 龍神さまからの見返しのことば。静かであたたかく、演劇的にしない",
    "miko_advice": "6. 巫女の助言。日々の行動に落とし込める短い実践助言",
    "things_to_remember": "7. 結び。静かに締め、強い販促や継続利用の押し付けはしない"
  }}
}}
...
見返し便としての7章構成:
1. 前回のお告げの振り返り: 前回鑑定結果を簡単に要約し、今回の相談テーマに関係する部分を優先して拾う。すべての占術結果を長く再説明しない。
...
- comparison_blocks は、相談テーマに応じた2〜3個の小見出しに絞り、各項目は「■ 個人事業は準備から改善の段階へ」のような見出しと、1つの短い文章ブロックにまとめる。
...
- 各章は長くしすぎず、要点はできるだけ箇条書きまたは短い段落で整理する。
...
- comparison_blocks は2〜3項目程度。テーマは近況メモと見返しテーマに応じて選び、「前回のお告げ」「現在の状況」「今回の読み直し」を見出しとして分割せず、1つの短い文章ブロックに統合する。当たった・外れたの判定はしない。
- next_3_month_action_items は3〜4項目程度。重大判断ではなく、前回鑑定の時間軸・占術的示唆・今回テーマを踏まえた鑑定上の助言として、今後3カ月で意識できる具体的な小さな行動にする。命令口調にしない。

重要な解釈方針:
- 見返し便では、前回鑑定の単なる要約や言い換えに終わらせない。
- 相談テーマと補足情報から外れた一般論を広げず、今回何を見返したいのかに関係する差分を優先する。
- 1章で前回のお告げをまとめた後は、2章以降で前回内容を細かく再説明しない。必要なときだけ「この流れ」「その課題」「前回から続くテーマ」のように受ける。
- 「前回のお告げで占術上示されていたこと」「今回の近況と、手相画像がある場合のみ現在の手相から見える現在の状態」「それを踏まえた今回の読み直し」は、内部では区別して考える。ただし本文では毎回ラベル分けせず、読みやすい短い文章に統合する。
...
- 現在の手相画像から見える印象は、今回の近況メモや見返しテーマとどう重なるか、これから3カ月の行動に関係しそうな手がかりは何かを整理する。
...
- 3章は具体的な行動に集中する。6章「巫女の助言」では同じ行動リストを繰り返さず、行動を続けるための日々の心がけや小さな工夫に寄せる。
...
- 「焦らず」「地道に」「着実に」「基盤固め」「長期的な視点」「不安は自然」「直感を信じる」「心身のバランス」「一歩一歩」など、同じ意味を持つ励まし表現の使用は本文全体で合計1〜2回までに抑える。同じ語を避けても、言い換えて意味を反復しない。
- 中盤のセクションでは、抽象的な励ましよりも、前回鑑定・現在の近況・今回のテーマ・手相画像がある場合のみ現在の手相画像から読み取れる内容の整理を優先する。
- 励まし要素は、主に「龍神さまからの見返しの言葉」「巫女の助言」「結び」に集約し、比較・分析セクションでは具体的な見返し内容を中心にする。
- 前回のお告げを「当たり」「外れ」と評価しない。
...
- things_to_remember には、今回の鑑定書を少し時間を置いて読み返すと今日とは違う点に目が留まることがある、という静かな結びを自然に短く含める。必要であれば、小さな行動を試した後の1〜3カ月ほどが読み返しの目安になる、という程度にとどめる。購入や継続利用を直接勧めない。
...
「龍神さまからの見返しの言葉」の文体:
- 神秘的な雰囲気を保ちつつも、過度に大げさ・宗教的・演劇的・断定的な表現は避ける。
- 「そなた」「龍神の力」「大いなる流れ」「大いなる潮流」「恐れることはない」「無限の可能性」「運命が動き出す」「龍神が示す」「龍神に導かれ」「豊かな未来が必ず」「試練」「使命」「魂」「奇跡」は使わない。
- 呼びかける場合は、必ず「〇〇さん」とさん付けにする。「そなた」のような古風な呼びかけは使わない。
- 「龍神が示す」ではなく、巫女が静かに読み直して伝える言葉として書く。
- 読者に圧を与える口調ではなく、静かであたたかく、前回からの歩みを見守るような励ましにする。
- 「龍神さまからの新たな導き」「新しいお告げが示された」のように新しい啓示を授ける表現は避け、「現在の流れをあらためて読み直す」「今の状況を静かに見返す」程度の表現にする。
- 前回鑑定の占術根拠や今回の近況を踏まえつつも、未来を断定せず、「〜のように見受けられます」「〜を意識するとよさそうです」「〜の流れが感じられます」などの柔らかい表現を使う。
