...
- comparison_blocks は、相談テーマに応じた2〜3個の小見出しに絞る。各項目は「■ 個人事業は準備から改善の段階へ」のような見出し、短い導入文1〜2文、要点の箇条書き3〜5個、短い読み直しのまとめ1〜2文で整理する。
...
- comparison_blocks は2〜3項目程度。テーマは近況メモと見返しテーマに応じて選び、「前回のお告げ」「現在の状況」「今回の読み直し」をラベルとして分割しない。各ブロックは、短い導入文、腹落ちする要点3〜5個、短い読み直しのまとめで構成する。当たった・外れたの判定はしない。
...
- 2章の箇条書きは単なるメモの羅列にしない。「広告を出した」「不安がある」のような事実だけで止めず、それが今の段階や相談テーマにどう関係するのかを短く添える。
