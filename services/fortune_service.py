...
正規PDFとして扱う対象:
- 通常版「龍神さまのお告げ」PDF
- 見返し便「龍神さまのお告げ 見返し便」PDF
- 過去に生成された正規の見返し便PDF（章名が現行版と異なる可能性があります）
...
- 展望：これから1年先の運勢
- 未来：2〜3年後の運勢
- 見返し便の場合は、前回のお告げ・今回の見返し・見返しテーマ・見返し系の複数章
- 巫女の助言

判定方針:
- 通常版13章の完全一致は必須にしないでください。
- detected_sections / missing_sections は参考情報です。missing_sections があっても、それだけで invalid にしないでください。
- 正規性は文書全体のブランド、鑑定日、フッター、本文構造、複数章の組み合わせで判断してください。
- タイトルだけを模倣したPDFは invalid としてください。
- すべての章が完全一致しなくても、通常版または見返し便の正規PDFとして十分に読める場合は valid としてよい。
