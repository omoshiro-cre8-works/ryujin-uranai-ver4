...
見返しサマリー、比較ブロック、3カ月の行動提案も、必ず占術上の示唆や前回鑑定の時間軸とのつながりを踏まえてください。全体の文章量は現在より1〜2割短い印象を目指し、PDFで4ページ前後、長くても5ページ程度に収まる密度を意識してください。
...
{{
  "review_fortune": {{
    ...
    "intro": "1. 前回のお告げの振り返り。今回の相談テーマに関係する部分を3〜5項目程度で短くまとめる",
    "continuing_flow": "2章の短い締め。小見出しで述べた内容を繰り返さず、2〜4文で全体の流れだけをまとめる",
    "current_changes": "2章の補足。今回の近況や手相画像から見える現在との関係だけを短く述べ、前回鑑定の再説明はしない",
    "theme_review": "2章の短い結び。相談テーマと補足情報に沿って、今あらためて見直すことを2〜4文でまとめる",
    "next_3_months": "3章の導入。具体行動は action_items に集約し、本文は1〜2文の短い方向づけにする",
    "one_year_guidance": "4. 1年先に向けて整えていくこと。強く断定せず、方向性を考えやすくする",
    "ryujin_message": "5章。現在の歩みを大きな方向性として静かに受け止める。結びと同じ励ましを繰り返さない",
    "miko_advice": "6章。3章の行動リストを再掲せず、日々続けるための小さな心がけや工夫を短く伝える",
...
- 各章は長くしすぎず、要点はできるだけ箇条書きまたは短い段落で整理する。全体として1〜2割短い印象を目指す。
...
- comparison_blocks は2〜3項目程度。テーマは近況メモと見返しテーマに応じて選び、「前回のお告げ」「現在の状況」「今回の読み直し」を見出しとして分割せず、1つの短い文章ブロックに統合する。当たった・外れたの判定はしない。各ブロックは3〜5文程度に抑える。
...
- 「前回のお告げで占術上示されていたこと」「今回の近況と、手相画像がある場合のみ現在の手相から見える現在の状態」「それを踏まえた今回の読み直し」は、内部では区別して考える。ただし本文では毎回ラベル分けせず、読みやすい短い文章に統合する。2章後半に総括を書く場合は2〜4文程度の短い締めにする。
...
- 3章は具体的な行動に集中する。説明文は1〜2文に抑え、決済改善、広告分析、転職活動の見直しなどの具体行動は箇条書きに集約する。6章「巫女の助言」では同じ行動リストを繰り返さず、行動を続けるための日々の心がけや小さな工夫に寄せる。
...
- 読者に圧を与える口調ではなく、静かであたたかく、前回からの歩みを見守るような励ましにする。結びで同じ内容を再度言い換えなくて済むよう、この章では大きな方向性だけを述べる。
