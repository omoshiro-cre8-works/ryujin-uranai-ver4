...
- 近況メモに書かれた本人の現実感。
- review_summary_points は4〜5項目程度。前回PDFを読み込んだからこそ分かる、前回のお告げ・占術上の示唆・現在の状況・相談テーマ・補足情報・時間軸のつながりを短く分かりやすく書く。抽象的な励ましだけにしない。
- comparison_blocks は原則3項目程度。テーマは近況メモと見返しテーマに応じて選び、「前回のお告げ」「現在の状況」「今回の読み直し」を必ず分ける。それぞれで、前回鑑定の占術的な示唆や、手相画像がある場合のみ今回の手相上の手がかりとの関係を踏まえる。当たった・外れたの判定はしない。
