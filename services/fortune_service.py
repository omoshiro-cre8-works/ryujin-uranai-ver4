...
def build_review_fortune_prompt(
    ...
    palm_image_instruction = (
        ...
        "前回PDF要約、時間軸再分類、相談テーマ、近況メモと重ねてください。本文では「今回の手相画像から読み取れる範囲では」「現在の手相からは」「今回添付された手のひらを見ると」「画像から確認できる範囲では」など、画像に基づく範囲を明示してください。"
...
見返し便としての構成方針:
- 通常版の占術説明を最初から繰り返さず、「前回のお告げ → 現在の状況 → 今回の読み直し → 今も活かせること → 見直した方がよいこと → これから3カ月の小さな行動」の流れを中心にする。
- 姓名判断・四柱推命・西洋占星術・手相は、前回PDFと今回状況を照らし合わせる根拠として必要な分だけ使う。
- 各セクションの役割を分ける。intro は見返しの焦点、continuing_flow は続く流れ、current_changes は今回の変化と手相、theme_review は選択テーマの分析、next_3_months と action_items は具体行動、one_year_guidance は中期の見通し、ryujin_message・miko_advice・things_to_remember は励ましと結びを担う。
...
- 手相の読み取りは断定せず、「画像から読み取れる範囲では」「〜のように見受けられます」「〜を示唆しているようです」「今回の近況メモと重ねると」「前回鑑定に記された傾向とも矛盾しない範囲で」など柔らかい表現にする。
...
- 「焦らず」「着実に」「基盤固め」「地道な努力」「長期的な視点」「不安は自然」「直感を信じる」「心身のバランス」「一歩一歩」などの表現は、必要な場面に絞って使い、各セクションで同じ言い回しを重ねない。
- 中盤のセクションでは、抽象的な励ましよりも、前回鑑定・現在の近況・今回のテーマ・手相画像がある場合のみ現在の手相画像から読み取れる内容の整理を優先する。
- 励まし要素は、主に「龍神さまからの見返しの言葉」「巫女の助言」「心に留めること」「結び」に集約し、比較・分析セクションでは具体的な見返し内容を中心にする。
- 前回のお告げを「当たり」「外れ」と評価しない。
...
- things_to_remember には、今回の鑑定書を手元に残し、少し時間を置いて読み返すと今日とは違う点に目が留まることがある、次に見返すなら小さな行動を試した後の1〜3カ月ほどが一つの目安になる、という静かな導線を自然に短く含める。購入や継続利用を直接勧めない。
